...
void main() {
  group('AuthInterceptor single-flight refresh', () {
    test(
      'concurrent 401 errors trigger exactly ONE refresh call',
      () async {
        final refresher = _SingleFlightRefresher();

        // Simulate a 200ms network refresh
        Future<void> slowRefresh() => Future.delayed(
        ...
        expect(
          ...
          1,
          reason: 'Only the first waiter must call doRefresh(); '
    ...
    test(
      'failed refresh propagates the error to all waiters',
      () async {
        final refresher = _SingleFlightRefresher();
  ...
  group('IdempotencyInterceptor key reuse contract', () {
    ...
    test('same idempotency key in extra produces identical Idempotency-Key header', () {
      ...
      final extra2 = <String, dynamic>{'idempotencyKey': key}; // same key, retry

      final header1 = extra1['idempotencyKey'] as String;
      final header2 = extra2['idempotencyKey'] as String;

      expect(header1, header2, reason: 'Key must not be regenerated on retry');
    });
